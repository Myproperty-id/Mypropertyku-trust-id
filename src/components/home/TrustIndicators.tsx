...
const statsData = [
  { value: 5000, suffix: "+", label: "Properti Terverifikasi", icon: CheckCircle },
  { value: 10000, suffix: "+", label: "Pengguna Aktif", icon: Users },
  { value: 50, suffix: "+", label: "Mitra Terpercaya", icon: Award },
];

const TrustIndicators = () => {
  const stats = statsData.map((stat) => ({
    ...stat,
    counter: useCountUp({ end: stat.value, suffix: stat.suffix }),
  }));
      ...
      <div className="container-main relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8 mb-16 md:mb-20">
          {trustFeatures.map((feature, index) => (
            <div
              key={index}
              className="card-3d p-6 group cursor-default"
